...
const BathroomAccessories = () => {
  ...
  useGSAP(() => {
    gsap.utils.toArray(containerRef.current.querySelectorAll('.accessory-image')).forEach((img) => {
      gsap.fromTo(img,
        { y: '5%' },
        {
          y: '-5%',
  ...
  useEffect(() => {
    ...
    const titleEl = el.querySelector('.accessory-title');
    const cards = el.querySelectorAll('.accessory-card');
    const viewBtn = el.querySelector('.accessory-view-btn');
  ...
  return (
    <section className="w-full bg-[#1F1F21] pt-0 pb-16 md:pb-24 px-4 sm:px-6 md:px-12">
      <div ref={containerRef} className="max-w-7xl mx-auto">
        {/* Section Title */}
        <h2 className="accessory-title text-2xl sm:text-3xl md:text-4xl lg:text-[2.5rem] font-medium text-white tracking-wide mb-8 md:mb-12 opacity-0 will-change-transform">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
          {products.map((product) => (
            <div
              key={product.id}
              className="accessory-card flex flex-col group cursor-pointer opacity-0 will-change-transform"
            >
        ...
        <div className="w-full flex justify-center mt-12 md:mt-14">
          <button
            className="accessory-view-btn flex items-center gap-3 px-8 py-3 border border-white/40

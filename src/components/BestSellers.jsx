  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (!alive || !settings?.bestseller_auto_enabled) return;
        setEnabled(true);
        const top = await productsApi.topViewed({ activeOnly: true, limit: 4 });
        if (alive) setProducts(top.filter((p) => p.view_count > 0));
      } catch {
        // Bestsellers is a bonus section — fail silently, just don't render it.
      }
    })();
    return () => { alive = false; };
  }, []);

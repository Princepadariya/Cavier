...
const OpalPrime = () => {
  ...
  useGSAP(() => {
    gsap.utils.toArray(containerRef.current.querySelectorAll('.opal-image')).forEach((img) => {
      gsap.fromTo(img,
        { y: '5%' },
        {
          y: '-5%',
  ...
  useEffect(() => {
    ...
    const titleEl = el.querySelector('.opal-title');
    const cards = el.querySelectorAll('.opal-card');
    const viewBtn = el.querySelector('.opal-view-btn');
  ...
  return (
    <section className="w-full bg-[#1F1F21] pt-0 pb-16 md:pb-24 px-4 sm:px-6 md:px-12">
      <div ref={containerRef} className="max-w-7xl mx-auto">
        {/* Section Title */}
        <h2 className="opal-title text-2xl sm:text-3xl md:text-4xl lg:text-[2.5rem] font-medium text-white tracking-wide mb-8 md:mb-12 opacity-0 will-change-transform">
          Opal Prime
        </h2>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
          {products.map((product) => (
            <div
              key={product.id}
              className="opal-card flex flex-col group cursor-pointer opacity-0 will-change-transform"
            >
